# RPLIDAR C1 激光雷达驱动程序（模块化网页版）

这是一个用于SLAMTEC RPLIDAR C1激光雷达的Python驱动程序，基于RPLIDAR协议v2.8实现，支持数据采集和**网页实时可视化显示**。检测范围：**12米**。

## 🎯 模块化特性（全新！）

本版本提供了**独立的JavaScript可视化模块**，可以轻松集成到任何网页中：

- 📦 **即插即用**：只需3行代码即可集成雷达显示
- 🎨 **高度可定制**：支持自定义颜色、大小、样式
- 🔌 **独立模块**：不依赖特定框架，可用于React、Vue等
- 📊 **完整功能**：包含实时显示、统计信息、连接管理
...
👉 **详细使用文档**：查看 [WIDGET_USAGE.md](./WIDGET_USAGE.md)
...
- ✅ 实时数据采集和解析
- ✅ 终端实时打印扫描数据（角度、距离、质量）
- ✅ 多线程并发处理（扫描线程 + WebSocket推送线程）
- ✅ 优雅的退出机制（Ctrl+C）
- ✅ 完善的异常处理

### 模块化可视化
- ✅ **独立JavaScript模块**（`rplidar-widget.js`）
- ✅ **原生Canvas绘制**（极坐标雷达图）
- ✅ **WebSocket实时通信**（Flask-SocketIO）
- ✅ **即插即用**（3行代码集成）
- ✅ **高度可定制**（颜色、大小、样式）
- ✅ **框架无关**（可用于React、Vue、Angular等）
- ✅ **多实例支持**（同页面显示多个雷达）
- ✅ **响应式设计**（支持移动端）
- ✅ **统计信息面板**（可选显示）
...
```
pyserial==3.5           # 串口通信
Flask==2.3.0            # Web服务器框架
Flask-SocketIO==5.3.0   # WebSocket实时通信
python-socketio==5.9.0  # SocketIO支持
```
...
雷达扫描已启动...
终端将每100个点打印一次数据
请在浏览器中访问 http://localhost:5000 查看实时可视化
按 Ctrl+C 停止
```
...
- Chart.js极坐标散点图显示360度扫描结果
- 实时WebSocket推送更新（50ms间隔）
- 显示实时统计信息：
  - 数据点数
  - 当前缓冲区大小
  - 平均/最小/最大距离
...
- `port=5000` - Web服务器端口（可修改为其他端口）
- `debug=False` - 生产模式（开发时可设为True）
...
```python
max_distance = 12.0      # 最大显示距离（米）- 12M检测范围
...
```python
# 在 _broadcast_loop 方法中
if current_time - last_broadcast_time >= 0.05:  # 50ms间隔
```
...
#### 2. `rplidar_visualizer.py` - 数据管理模块

**主要类**: 
- `RadarDataManager` - 雷达数据缓存和管理
- `TerminalPrinter` - 终端数据打印

**数据管理功能**:
```python
update_data(angle, distance, quality)  # 更新数据点
get_scan_data()                         # 获取所有扫描数据
get_latest_point()                      # 获取最新数据点
...
#### 3. `main.py` - 主程序（Flask Web应用）

**主要组件**: 
- `RadarApplication` - 雷达应用类
- Flask应用和路由
- SocketIO事件处理
...
5. 创建数据处理线程
6. 创建WebSocket广播线程
7. 启动Flask Web服务器（阻塞主线程）
8. Ctrl+C退出，清理资源
```

**多线程架构**:
- 主线程：Flask服务器（http://localhost:5000）
- 扫描线程：持续读取串口数据
- 广播线程：每50ms向WebSocket客户端推送数据
- 线程安全：使用锁保护共享数据

**路由和事件**:
```python
@app.route('/')              # 主页，返回可视化HTML
@socketio.on('connect')      # 客户端连接事件
@socketio.on('disconnect')   # 客户端断开事件
@socketio.on('request_stats') # 请求统计信息
```

#### 4. `templates/index.html` - 网页可视化界面
